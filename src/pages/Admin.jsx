...
const Admin = () => {
    ...
    const handleDeleteQuote = async (id) => {
        if (!window.confirm('Confirma exclusão deste orçamento?'))
            return;
    ...
    return (<div className="page-shell">
      ...
      <main className="admin-page">
        <section className="admin-panel">
          ...
          {loading ? (<p style={{ textAlign: 'center', padding: '40px 0', opacity: 0.7 }}>Carregando serviços...</p>) : (<>
              ...
              {services.filter(s => !['garcons', 'recepcionistas', 'djs', 'decoracao', 'salao'].includes(s.id)).map(service => {
                ...
                return (<article key={service.id} className={`admin-card ${isCollapsible && isCollapsed ? 'collapsed' : ''}`}>
                    <div className={`admin-card-title ${isCollapsible ? 'clickable' : ''}`} onClick={() => {
                        if (!isCollapsible)
                            return;
                    ...
                    }} role={isCollapsible ? 'button' : undefined} tabIndex={isCollapsible ? 0 : -1}>
                      <h3>{service.title}</h3>
                    ...
                    {!(isCollapsible && isCollapsed) && (<>
                        {!['sonorizacao', 'iluminacao'].includes(service.id) ? (<>
                            <label>
                              Nome do serviço
                              <input value={service.title} onChange={e => handleChange(service.id, 'title', e.target.value)}/>
                            </label>
                            <label>
                              Descrição
                              <textarea rows={3} value={service.description} onChange={e => handleChange(service.id, 'description', e.target.value)}/>
                            </label>
                          </>) : null}

                        {service.options?.length ? (<div className="admin-options">
                            <h4>Pacotes / opções</h4>
                            {service.options.map((option, index) => (<div key={`${service.id}-${index}`} className="admin-option-editor">
                                <label>
                                  Nome do pacote
                                  <input value={option.label} onChange={e => handleOptionChange(service.id, index, 'label', e.target.value)}/>
                                </label>
                                <label>
                                  Descrição
                                  <textarea rows={2} value={option.description} onChange={e => handleOptionChange(service.id, index, 'description', e.target.value)}/>
                                </label>
                                <label>
                                  Valor por hora
                                  <input type="number" value={option.price} onChange={e => handleOptionChange(service.id, index, 'price', e.target.value)}/>
                                </label>
          ...
          <article className="admin-card">
            ...
            {quoteExists && storedQuote ? (<>
                ...
                return (<div key={item.id} className={`admin-quote-item ${isExpanded ? 'expanded' : ''}`}>
                        <button type="button" className="admin-quote-item-toggle" onClick={() => toggleQuoteItem(item.id)}>
                          <span>{item.title}</span>
                          <span className="admin-quote-item-hint">{isExpanded ? 'Ocultar' : 'Ver'}</span>
                        </button>
                        {isExpanded && (<div className="admin-quote-item-details">
          ...
          <article className="admin-card">
            ...
            {loadingSavedQuotes ? (<p style={{ textAlign: 'center', padding: '24px 0', opacity: 0.6 }}>Carregando orçamentos...</p>) : savedQuotes.length ? (<div className="admin-quote-items">
                {savedQuotes.map(q => (<div key={q.id} className="admin-quote-item">
                    ...
                    {quoteDetailsOpen[q.id] && (<div className="admin-quote-item-details">
                        ...
                        <div style={{ borderTop: '1px solid rgba(255, 184, 77, 0.2)', paddingTop: '12px' }}>
                          ...
                          {(q.quote?.items || []).map((item) => (<div key={item.id} style={{ marginBottom: '10px', paddingBottom: '10px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              </div>) : (<p style={{ textAlign: 'center', padding: '24px 0', opacity: 0.6 }}>📭 Nenhum orçamento salvo no banco de dados</p>)}
          </article>
        </section>
      </main>
    </div>);
