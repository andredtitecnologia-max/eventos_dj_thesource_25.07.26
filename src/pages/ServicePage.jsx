    ...
    return (<div className="page-shell">
      ...
      <main className="service-page">
        <section className="service-details">
          <div className="service-content">
            ...
            {service.hourly ? (<>
                ...
                {service.options?.length ? (<>
                    <label>
                      ...
                      <select value={selectedOptionIndex} onChange={e => setSelectedOptionIndex(Number(e.target.value))}>
                        {service.options.map((option, index) => (<option key={option.label} value={index}>
                            {option.label} — R$ {option.price.toFixed(2)}/h
                          </option>))}
                      </select>
                    </label>
                    {selectedOption && (<p className="service-option-description">{selectedOption.description}</p>)}
                  </>) : (<label>
                    Valor por hora:
                    <select value={priceIndex} onChange={e => setPriceIndex(Number(e.target.value))}>
                      {service.values.map((value, index) => (<option key={value} value={index}>
                          R$ {value.toFixed(2)}
                        </option>))}
                    </select>
                  </label>)}
                ...
                <select value={priceIndex} onChange={e => { setPriceIndex(Number(e.target.value)); setQuantity(guestLabels[Number(e.target.value)]); }}>
                  {guestLabels.map((label, index) => (<option key={label} value={index}>R$ {service.values[index].toFixed(2)} — {label} convidados</option>))}
                </select>
              </label>)}
            {service.id === 'salao' && (<div className="salon-fields">
                <label>
                  Tema do evento
                  <input type="text" value={eventTheme} onChange={e => setEventTheme(e.target.value)} placeholder="Ex: Casamento, Aniversário"/>
                </label>
                <label>
                  Endereço do evento
                  <input type="text" value={eventAddress} onChange={e => setEventAddress(e.target.value)} placeholder="Rua, número, bairro"/>
                </label>
                <label>
                  Horário de início
          ...
          {service.id === 'sonorizacao' && (<div className="service-image-container">
              <div className="image-frame">
                <img src={`${baseUrl}audio-equipment.png`} alt="Equipamento de sonorização" className="service-title-image"/>
              </div>
            </div>)}
          {service.id === 'iluminacao' && (<div className="service-image-container">
              <div className="image-frame">
                <img src={`${baseUrl}sonorizacao-equipment.png`} alt="Equipamento de iluminação" className="service-title-image"/>
              </div>
            </div>)}
          {service.id === 'garcons' && (<div className="service-image-container">
              <div className="image-frame">
                <img src={`${baseUrl}garcons.jpg`} alt="Garçons profissionais" className="service-title-image"/>
              </div>
            </div>)}
          {service.id === 'djs' && (<div className="service-image-container">
              <div className="image-frame">
                <img src={`${baseUrl}djs.jpg`} alt="DJ profissional" className="service-title-image"/>
              </div>
            </div>)}
          {service.id === 'decoracao' && (<div className="service-image-container">
              <div className="image-frame">
                <img src={`${baseUrl}decorador.jpg`} alt="Decorador profissional" className="service-title-image"/>
              </div>
            </div>)}
          {service.id === 'salao' && (<div className="service-image-container">
              <div className="image-frame">
                <img src={`${baseUrl}salaoeventos.jpg`} alt="Salão de eventos" className="service-title-image"/>
              </div>
            </div>)}
          {service.id === 'recepcionistas' && (<div className="service-image-container">
              <div className="image-frame">
                <img src={`${baseUrl}recepcionista.jpg`} alt="Recepcionistas profissionais" className="service-title-image"/>
              </div>
            </div>)}
        </section>
      </main>
